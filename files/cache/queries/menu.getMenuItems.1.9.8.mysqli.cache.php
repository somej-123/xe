...
${'menu_srl8_argument'} = new ConditionArgument('menu_srl', $args->menu_srl, 'equal');
${'menu_srl8_argument'}->checkFilter('number');
${'menu_srl8_argument'}->checkNotNull();
${'menu_srl8_argument'}->createConditionValue();
if(!${'menu_srl8_argument'}->isValid()) return ${'menu_srl8_argument'}->getErrorMessage();
if(${'menu_srl8_argument'} !== null) ${'menu_srl8_argument'}->setColumnType('number');
if(isset($args->parent_srl)) {
${'parent_srl9_argument'} = new ConditionArgument('parent_srl', $args->parent_srl, 'equal');
${'parent_srl9_argument'}->checkFilter('number');
${'parent_srl9_argument'}->createConditionValue();
...
${'sort_index10_argument'}->ensureDefaultValue('listorder');
...
new ConditionWithArgument('`menu_srl`',$menu_srl8_argument,"equal")
,new ConditionWithArgument('`parent_srl`',$parent_srl9_argument,"equal", 'and')))
));
