...
${'var_idx25_argument'} = new ConditionArgument('var_idx', $args->var_idx, 'equal');
${'var_idx25_argument'}->checkFilter('number');
${'var_idx25_argument'}->createConditionValue();
if(!${'var_idx25_argument'}->isValid()) return ${'var_idx25_argument'}->getErrorMessage();
} else
${'var_idx25_argument'} = NULL;if(${'var_idx25_argument'} !== null) ${'var_idx25_argument'}->setColumnType('number');

${'sort_index26_argument'} = new Argument('sort_index', $args->{'sort_index'});
${'sort_index26_argument'}->ensureDefaultValue('var_idx');
...
,new ConditionWithArgument('`var_idx`',$var_idx25_argument,"equal", 'and')))
));
